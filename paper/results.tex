...
\paragraph{Recalibration is a decision, not a default.}
...
\NUM{isoworseathigh} at 100.  The average is carried by the single worst-
calibrated site: where the cliff is a factor of ten, a fitted map removes most
of it; where calibration was already acceptable, the same map adds more
...
The same labels that would fit a map can decide whether to fit one.  Selecting
among candidates --- with the identity always among them --- by cross-validation
on the calibration set picks a map where there is a cliff and leaves the model
alone where there is not.  In controlled simulation this is the difference
between a nine-fold degradation and none: at a site with no cliff, isotonic
takes ECE from 0.0019 to 0.0178 while selection returns the model unchanged.
We regard this, rather than any particular recalibration family, as the
