...
can use its own unlabeled recordings to rule the free correction \emph{out} ---
...
\emph{The minimax correction is the safest unlabeled default, under a stated
condition.}  Replacing the gate's binary decision with an optimisation over the
identified set gives a method that, whenever the source model satisfies
Assumption~\ref{ass:srccal}, was never worse than shipping unchanged in any
regime we tested: best under pure label shift, and better than the gate wherever
concept shift is mild.  The condition is not decorative.  In a deliberately
under-powered configuration whose source calibration split was too small to
establish source calibration --- measured source ECE 0.015--0.053, with a source
operator two to eight times worse conditioned --- \emph{every} unlabeled
correction, minimax included, left calibration worse than shipping unchanged.
Assumption~\ref{ass:srccal} is the precondition of the whole decomposition, and
a site that cannot establish it on its own source data should not expect the
free correction to help.
