...
\begin{remark}[What the decomposition buys, and what it does not]
$D_{\mathrm{label}}$ is a functional of $(\pi_s,\pi_t,\mu_t)$ alone, so
correcting it requires exactly one unknown quantity: the target prevalence.
That is a far weaker requirement than fitting a recalibration map, and it is the
source of the decomposition's practical value.

It is emphatically \emph{not} the same as requiring no labels.  Removing
$D_{\mathrm{label}}$ without labels requires $\pi_t$ to be identified from
unlabeled data, which Theorem~\ref{thm:ident} shows holds only under
conditions that cross-national transfer violates --- and violates in the
direction (\S\ref{sec:results-recal}) where no unlabeled test can detect it.  A
prevalence measured on a small labelled sample is identified unconditionally,
because a proportion needs no model.  The distinction between ``needs one
scalar'' and ``needs no labels'' is the difference between a deployable
procedure and an unsafe one, and \S\ref{sec:results-recal} reports which side
of it the unlabeled route falls on.

$D_{\mathrm{concept}}$ is by Theorem~\ref{thm:decomp}(c) precisely what survives
the best possible prior correction, and no unlabeled procedure can estimate it:
two targets with the same $\mu_t$ can have any $g_t$ whatsoever.
\end{remark}
...
better calibrated than either component alone implies, and applying the prior
...
\begin{remark}[What the test can and cannot see]\label{rem:gofblind}
The statistic measures the distance from $\hat q_t$ to the cone
$\{Aw : w \ge 0\}$, so it detects concept shift that moves the target
histogram \emph{off} that cone and is blind to shift that moves it
\emph{along} it.  This is not a deficiency of the statistic; by
Theorem~\ref{thm:ident} the along-cone component is exactly the part that is
not identified, and no unlabeled procedure can recover it.  Passing the test is
therefore necessary evidence, not sufficient.

The gap is not hypothetical.  In our transfer matrix one site--label pair
returned an unlabeled prevalence estimate of $0.85$ against a true prevalence of
$0.02$ with a $p$-value of $0.23$: the concept shift there happened to be almost
entirely along the cone, and was invisible.  A blunt plausibility bound on the
implied change in prevalence catches that class of failure where the test
structurally cannot, and the released software applies one --- labelled as the
sanity check it is, not as an inference.  It is also why we recommend the
minimax correction of \S\ref{sec:results-recal} over a gate that trusts the
test's verdict: a procedure that bounds the damage is worth more than one that
can be silently fooled.
\end{remark}

Proposition~\ref{prop:gof} is nonetheless the most immediately deployable
statement in this paper: a hospital can evaluate it on its own unlabeled ECGs, before any chart
review, and learn whether its shift is reducible to prevalence at all.  As
Remark~\ref{rem:gofblind} and \S\ref{sec:results-recal} record, a
non-rejection does not license an unlabeled correction; a rejection does rule
one out.  Two implementation points matter and are documented
...
$n\ge(1-2\alpha)^{2}/(8\varepsilon^{2})$.
If $\Gamma<\varepsilon$, no such lower bound exists: $n^\star=0$, because the
residual left after the prior correction already meets the budget.  Note this
says nothing about how $\pi_t$ was obtained --- $\Gamma$ is defined with respect
to the correction actually applied.
\end{enumerate}
\end{theorem}
...
tensorisation give
$\mathrm{TV}(P_0^{\otimes n},P_1^{\otimes n})^2\le \tfrac{n}{2}\mathrm{KL}(P_0\|P_1)$,
and for Bernoulli conditionals separated by $2\varepsilon$ about a common
success probability $p$,
$\mathrm{KL}(P_0\|P_1)\le 4\varepsilon^{2}/\big(p(1-p)\big)$.  Choosing the hard
instance at $p=1/2$ maximises $p(1-p)$ and gives
$\mathrm{TV}^2 \le 8n\varepsilon^{2}$, whence
$n\ge(1-2\alpha)^{2}/(8\varepsilon^{2})$.  (Note the exponent: one cannot pass
from $(1-2\alpha)^{2}$ to $(1-2\alpha)$, since squaring a number in $(0,1)$
makes it smaller and the weaker expression would \emph{overstate} what has been
proved necessary.)  For the threshold: both hypotheses must lie in the class of
...
\begin{remark}[The two bounds are complementary, not a bracket]\label{rem:regimes}
It is worth being explicit that (a) and (c) do not apply in the same regime, and
so do not sandwich a common quantity.  The sufficient bound is finite only for
$\Gamma<\varepsilon$ and diverges as $\Gamma\uparrow\varepsilon$, because a
one-parameter family has no capacity left once the residual shift consumes the
budget.  The necessary bound is non-vacuous only for $\Gamma\ge\varepsilon$,
because below that no hard instance fits inside the admissible ball.  Together
they partition the parameter space at $\Gamma=\varepsilon$:

\begin{center}
\begin{tabular}{lll}
\toprule
regime & one-parameter recalibration & any procedure \\
\midrule
$\Gamma \ll \varepsilon$ & $n=0$: already within budget & $n=0$ \\
$\Gamma < \varepsilon$ & $n \le z^2 L^2 / \big(I(\varepsilon-\Gamma)^2\big)$ & --- \\
$\Gamma \ge \varepsilon$ & unattainable at any $n$ & $n \ge (1-2\alpha)^2/(8\varepsilon^2)$ \\
\bottomrule
\end{tabular}
\end{center}

What \emph{is} matched is the rate: both sides scale as the inverse square of
the budget actually available, $(\varepsilon-\Gamma)^{-2}$ above and
$\varepsilon^{-2}$ below.  We claim the exponent, not the constant.  An earlier
draft of this paper reported the two bounds as agreeing to within a few per
cent; that comparison evaluated them at different $\Gamma$ and at different
confidence levels, and was meaningless.
\end{remark}
...
\]
If $\alpha'>0$ then target coverage is at least $1-\alpha$, exactly in the
randomised form of the procedure (each calibration point drawn from the target
pool with probability $\lambda$) and up to an $O\big((n+m)^{-1}\big)$ term in the
deterministic weighted form that the software implements.
\item \textbf{(Efficiency.)}  The hybrid quantile's sampling variance is
proportional to $\lambda^2/n + (1-\lambda)^2/m_{\mathrm{eff}}$, against $1/n$
for target-only conformal, where $m_{\mathrm{eff}}$ is the weighted effective
source size.  For $n$ small and $m_{\mathrm{eff}}\gg n$ this is smaller by a
factor of roughly $\lambda^{2}$.
\end{enumerate}
\end{theorem}
...
(b) Consider first the randomised form, in which each calibration point is drawn
from $P_t$ with probability $\lambda$ and from $P_{s\to t}$ otherwise.  Its
calibration points are then i.i.d.\ from
$Q_\lambda=\lambda P_t+(1-\lambda)P_{s\to t}$ and exchangeable with one another,
so split conformal at level $\alpha'$ has coverage $\ge 1-\alpha'$ under
$Q_\lambda$.  Since $\mathrm{TV}(P_t,Q_\lambda)\le(1-\lambda)\gamma$, part (a)'s
argument transfers it to $P_t$ at a cost of at most $(1-\lambda)\gamma$, giving
coverage $\ge 1-\alpha'-(1-\lambda)\gamma=1-\alpha$.

The deterministic weighted form used in practice places exactly $\lambda$ mass
on the target points rather than a $\mathrm{Binomial}(n+m,\lambda)$ share of
them.  Its weighted empirical distribution converges to $Q_\lambda$ at rate
$O((n+m)^{-1/2})$ and its quantile to the corresponding quantile of $Q_\lambda$,
so the guarantee holds up to an $O((n+m)^{-1})$ correction in the coverage level
rather than exactly.  We state both because the distinction is the difference
between a finite-sample guarantee and an asymptotic one, and only the randomised
version has the former.

(c) The quantile is an order statistic of a weighted empirical distribution, so
its sampling variance scales with the sum of squared weights.  With mass
$\lambda$ spread over $n$ target points and $1-\lambda$ over $m_{\mathrm{eff}}$
effective source points, that sum is
$\lambda^{2}/n+(1-\lambda)^{2}/m_{\mathrm{eff}}$, which is the stated rate.
\end{proof}
