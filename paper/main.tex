...
and a concept-shift component.  The split is an exact identity in squared
calibration error, and the label-shift half is large: correcting for the target
prevalence alone removes \NUM{oraclegainpooled} of the calibration error, and
\NUM{oraclegainbest} at the worst-hit site.

We then report a negative result that changes what to do with it.  That half is
\emph{not} free.  Recovering the target prevalence from unlabeled data requires
the label-shift assumption, and cross-national shift violates it in the one
direction no unlabeled procedure can detect --- along the label-shift cone,
where we prove the prevalence is unidentified and where a goodness-of-fit test
is structurally blind.  Applied with an unlabeled prevalence estimate, the
correction made calibration substantially \emph{worse} than shipping the model
unchanged at every site we tested.  A second finding follows from the same tables and points the same way.  Averaged
over sites a fitted recalibration map beats shipping unchanged at every label
budget; broken out by site and label it is \emph{worse} in a majority of pairs,
the average carried by the single worst-calibrated site.  At sites already
acceptably calibrated a fitted map adds more estimation noise than it removes
bias, and at our same-country control it makes calibration measurably worse.
Recalibration is therefore a decision rather than a default, and a small
labelled sample is enough to make it: cross-validated selection with ``do
nothing'' among the candidates acts where there is a cliff and leaves the model
...
\item \textbf{Theorem~\ref{thm:ident} (identification), and a negative result.}
Necessary and sufficient conditions for that split to be identified, with
black-box shift estimation recovered as the degenerate case and an anchor
relaxation checkable on source data.  Empirically the conditions fail across
countries in the direction unlabeled data cannot see, so the unlabeled
correction is unsafe --- and measuring the prevalence on a few dozen labels is
...
in advance.
\item \textbf{A decision procedure, not a method.}  Cross-validated selection
over a candidate set that includes the identity, which recalibrates where there
is a cliff and declines where there is not --- and which declines to compare at
all when the calibration sample is too small to support an honest comparison.
